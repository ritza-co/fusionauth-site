...
- `export-code-examples.yml` — Pushes each local code example to its readonly external repository. Each repository must have a `repositoryUrl.txt` file that says what URL (without the `https://` and ending in `.git`) to export the files. Set `repositoryUrl.txt` to `internal` to skip export for self-contained guides. The Bluehawk annotations, `repositoryUrl.txt` file, and `tests` folder are removed before publishing.
