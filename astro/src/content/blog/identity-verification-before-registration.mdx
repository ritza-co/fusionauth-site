...
* an API key from Fideo, which you can get by [contacting them](https://www.fideo.ai/contact-us/) or [starting a free trial](https://app.fideo.ai/register/offer/fusionauth)
...
Don't forget to configure the following environment variables:

- `LICENSE_KEY`: the FusionAuth license key
- `FIDEO_API_KEY`: your Fideo API Key
...
* an admin user with an email of "<LocalValue path="example-kickstart/identity-verification/kickstart.json" selector="$.variables.adminEmail" />" and a password of "<LocalValue path="example-kickstart/identity-verification/kickstart.json" selector="$.variables.adminPassword" />"
...
<LocalCode src="example-kickstart/identity-verification/lambdas/fideo.js.snippet.validate.js" language="javascript" />
...
<LocalCode src="example-kickstart/identity-verification/lambdas/fideo.js.snippet.failthreshold.js" language="javascript" />
...
The error the user sees is stored in the [`messages.properties` file](/docs/customize/look-and-feel/advanced-themes#displaying-messages). This file contains all of the messages displayed to end users. It can be customized and localized as well.
...
Open up the [admin UI](http://localhost:9011). Use an incognito window or a different browser than you used to test the registration process. Log in as the admin user, with an email of "<LocalValue path="example-kickstart/identity-verification/kickstart.json" selector="$.variables.adminEmail" />" and a password of "<LocalValue path="example-kickstart/identity-verification/kickstart.json" selector="$.variables.adminPassword" />".
...
Save the new lambda code by clicking the <Breadcrumb>Save</Breadcrumb> button at the top right.
...
You should measure the success and failure of your registration rates. The success rate can be tracked by capturing the [User Registration Create webhook](/docs/extend/events-and-webhooks/events/user/registration/user-registration-create). Tracking the failures is a bit trickier. You have two options, explored below. 
...
<LocalCode src="example-kickstart/identity-verification/lambdas/fideo.js.snippet.registrationfailfetch.js" language="javascript" />
...
<LocalCode src="example-kickstart/identity-verification/lambdas/fideo.js.snippet.registrationfaillog.js" language="javascript" />
