...
For this example, the [FusionAuth Express Starter](https://github.com/FusionAuth/fusionauth-example-express-start-here) repository is extended to include a settings page where users can manage their MFA settings. The complete application can be found [here](https://github.com/FusionAuth/fusionauth-example-express-mfa)

### Setting Up MFA from the Admin Dashboard

MFA in FusionAuth can be configured at the **tenant level**, meaning all applications under a tenant inherit its MFA settings by default. If you want to configure MFA at the application level, this requires the **Enterprise plan**. Application level settings will take precedence if configured. [Learn more about application-specific MFA settings](/docs/lifecycle/authenticate-users/multi-factor-authentication).
