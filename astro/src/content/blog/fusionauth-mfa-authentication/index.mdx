...
MFA in FusionAuth is configured at the **tenant level**, meaning all applications under a tenant inherit its MFA settings by default. If you want to configure MFA at the application level, this requires the **Enterprise plan**. [Learn more about application-specific MFA settings](/docs/lifecycle/authenticate-users/multi-factor-authentication#application-set-up).
