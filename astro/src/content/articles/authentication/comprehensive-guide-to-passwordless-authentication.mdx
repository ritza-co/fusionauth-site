...
- [What Authentication Method Should You Choose?](#what-authentication-method-should-you-choose)
- [Further Reading](#further-reading)

## Introduction

This article explains the alternatives to traditional username/password login, how they work technically, when to use them, and how difficult they are for both your users to use and your programmers to program. By the end of this article, you should be able to answer questions like:

- How exactly is a passkey different from a password?
...
- What is the real benefit to passkeys, and what does the actual configuration and code look like to implement them?
- Should my website use multiple authentication methods or just one, and how do I decide?

## What's Wrong With Passwords?

If you're conscientious, you might create long random unique passwords on every site you visit, keep them securely in an encrypted document or password manager, and never share them. Unfortunately the average user doesn't do this. Below are all the difficulties and dangers of passwords as they are used today.

- **Inconvenience**: Websites have onerous requirements to create passwords, that differ from site to site, and are often cryptographically not even secure. Users might also be asked to change their passwords frequently.
- **Forgetfulness**: Users forget their passwords, especially if they have multiple accounts or don't use certain accounts often.
- **Reuse**: Users use the same password for many sites. If one site is hacked, all others using the same password are in danger.
- **Lockout**: A user can lose access to recovery email address, losing their account forever if they forget their password for the website.
- **Guessable**: Weak passwords are easy for hackers to guess or brute-force using dictionary attacks or rainbow tables.
- **Theft**: Even with a strong password, a user can be tricked into revealing a password on a fake website (phishing), by social engineering, physical theft, shoulder surfing, key logging, or an evil maid attack.
- **Breaches**: If hackers gain access to a website database, all customers' passwords are in danger. This is worse if the website didn't hash the passwords, or hashed them with weak algorithms.
- **Sharing**: Employees in workplaces share passwords, causing security risks (including sabotage), and making auditing and non-repudiation impossible. For paid services like Netflix, sharing passwords also makes fraud between friends easy and fun.

In summary, passwords fail at **convenience** and **security** for non-technical users and companies.

## What Is Passwordless Authentication?

To overcome the problems above, security experts developed these common password alternatives: passkeys, magic links, and multi-factor authentication (MFA).
...
In 2013 the biggest USA software companies formed the FIDO Alliance (**F**ast **ID**entity **O**nline) to develop and promote authentication standards that "*help reduce the world’s over-reliance on passwords*". Their standard, called FIDO2, was submitted to the World Wide Web Consortium (W3C) and became the WebAuthn standard in 2016. WebAuthn is a protocol that describes how clients and servers can authenticate using asymmetric cryptography (called passkeys here) instead of passwords.
...
Note the word "browser", not "user". The user doesn't do anything in this process, except consent to register or authenticate. The user doesn't make a password — the browser makes a passkey. So the authentication process is ultimately between a server and a device, not a server and a user. This implies that if two users share a computer, they could not log in to the same site with different usernames and passwords, because there are no longer usernames or passwords in WebAuthn.

Because the user has almost nothing to do in this process, passkeys are said to be frictionless login. All the user will see is something like the screenshot below.
...
You probably have the following questions if you're seeing this protocol for the first time:
...
Using a magic link means that a user doesn't need a password or a passkey; they need only an email address. The disadvantage is whenever the user's session expires, they will have to request a new link, open their email client, click the link, and delete the email. It's more convenient than a password for most users, unless they frequently log out of sites, or the website sets a very short session duration.
...
In the latter case, an OTP is called multi-factor authentication (MFA) or two-factor authentication (2FA). This isn't passwordless authentication, as you still have to enter a password, with the extra effort of entering an OTP too. However, entering an OTP does reduce some security flaws of passwords. More security, but less convenience.
...
Let's consider how the three passwordless (or password-extra) methods — passkeys, magic links, and MFA — solve the password problems listed earlier. This list can be separated into three groups of problems:
1. Inconvenience
...
Passkeys require almost no effort from the user and, since private keys never leave the device unencrypted, solve the second group of problems too.

However, passkeys are still vulnerable to theft — physical theft of a phone, master password, key logging or faking a user's face — and lockout — losing access to a master password or device.
...
If passkeys are endorsed by the software giants, and solve most of the problems of passwords, why do so few sites use them? What are the problems?

### Support

Operating systems may not support them. For instance, all users of Linux and users of older versions of Windows that don't log in to an online account don't have passkey support. Below is the confusing screen you see if you try to use a passkey website on Ubuntu.
...
Similarly, if a user loses access to their master password, they are locked out of every account.

In contrast, if a user uses different passwords for every site, an attacker gaining access to one site gives them no access to anything else. Forgetting a password for one site doesn't lock you out of all others.
...
If you don't have to use passwords, for public websites you should always use magic links instead of passwords. Passwords offer almost no advantages. Choosing between magic links and passkeys is more difficult. If possible, provide both options to users to support those who may not be able or willing to use passkeys.

If you want to use only passkeys for the security advantages, you should provide a brief explanation of what they are on your registration page, as well as explain to users the devices they can use, and how they can synchronize their passkeys to other computers.
...
There are some use cases where specific authentication methods are required. For devices without keyboards (TVs, fridges, and cars), passkeys are the only option. Highly regulated industries, such as banking and large corporations, might require passkeys.
...
Let's see which types of passwordless authentication FusionAuth offers, and briefly overview how to configure them. Once you're done with article, you might want to read more detail in the official FusionAuth [documentation on passwordless authentication](https://fusionauth.io/docs/lifecycle/authenticate-users/passwordless/).

<Aside type="tip">
If you want to test these options yourself, you'll need an installation of FusionAuth. The easiest way to run FusionAuth is in a Docker container. Clone the example GitHub repository [here](https://github.com/fusionauth/fusionauth-example-docker-compose). Open a terminal in the `light` subdirectory, and run `docker compose up` in a terminal to start FusionAuth. Log in at http://localhost:9011 with `admin@example.com` and `password`.
...
FusionAuth supports magic links. You can configure FusionAuth to allow a user to log in with magic links in addition to, or as an alternative to, username and password authentication.
...
For more information on magic links in FusionAuth, read the [documentation](https://fusionauth.io/docs/lifecycle/authenticate-users/passwordless/magic-links).
...
A TOTP authentication flow is similar to an OTP. The user enters an OTP, that changes every thirty seconds, on the FusionAuth login page from an app on their phone. The user configures TOTPs by entering a secret key shared between FusionAuth and an authenticator app on their phone. All apps use the same protocol, so the user can choose their favourite app. However, a phone must be used, because FusionAuth presents the shared secret as a QA code that has to be photographed.
...
The response above contains the code (OTP) that the user will be challenged to enter. You need to ask the user to enter this code on your website. The two factor Id is a field that use you in subsequent FusionAuth calls to remember what MFA flow you're busy with.

Next, as FusionAuth to send the OTP to the user's email. You specify only the two factor Id, not the code itself.
...
official documentation [here](https://fusionauth.io/docs/lifecycle/authenticate-users/passwordless/webauthn-passkeys#authentication-2).
...
Open a terminal in that directory and run the command below to serve the page at http://localhost:3000 using Docker.
...
Start the passkey re-authentication process using the curl command below. You might need to replace the `userId` value with the value of the user you created to test with.
...
Copy the text that appears in the lower textbox, paste it between the `'` quotes at the end of the command below, overwriting the example payload shown, and run the command.
...
`{"fieldErrors":{},"generalErrors":[{"code":"[Exception]","message":"FusionAuth encountered an unexpected error. Please review the troubleshooting guide found in the documentation for assistance and the available support channels."}]}`. To find the problem, you will need to compare the JSON you are sending to the FusionAuth [API specification](https://fusionauth.io/docs/apis/webauthn#complete-a-webauthn-passkey-assertion), and to the expected [example JSON parameter](https://fusionauth.io/docs/lifecycle/authenticate-users/passwordless/webauthn-passkeys#completing-the-assertion), manually. There is a [bug](https://github.com/FusionAuth/fusionauth-issues/issues/3001) logged to fix the vague error message, which is internally a null pointer exception.
</Aside>
...
- https://developer.mozilla.org/en-US/docs/Web/API/Web_Authentication_API, https://jsr.io/@simplewebauthn/browser/doc
