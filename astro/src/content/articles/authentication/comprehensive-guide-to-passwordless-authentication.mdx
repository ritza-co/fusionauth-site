...
To configure magic links, follow the [guide](https://fusionauth.io/docs/lifecycle/authenticate-users/passwordless/magic-links).

All you have to do is ensure that you have an email template ready to send the user (which was done for you in the [Kickstart file of the repository](https://github.com/fusionauth/fusionauth-example-docker-compose/blob/main/light/kickstart/kickstart.json) you downloaded) and enable passwordless login in the security tab of the "application" settings (an application is the FA name for a group of users and how they are allowed to log in).
...
FusionAuth doesn't support the former case. There isn't usually a need for this, as the process is effectively identical to using a magic link for login.

In the latter case an OTP is called multi-factor authentication (MFA) or two-factor authentication (2FA). This isn't passwordless authentication as you still have to enter a password, with the extra effort of entering an OTP. However, entering an OTP does reduce some security flaws of passwords.
