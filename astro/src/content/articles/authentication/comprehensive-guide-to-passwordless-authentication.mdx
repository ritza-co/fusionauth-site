...
If your site is dangerous (banking or medical), require your users to use MFA. Ensure MFA is as convenient as possible by providing users with a choice of delivery options, such as email, SMS, WhatsApp, or TOTP.
...
For extreme cases of life and death, like pacemakers or aircraft security devices, you need to balance the extra security of passkeys with the need to grant immediate access during an emergency (which is easier with a password) and the risk of account lockout if keys are lost.
...
To configure magic links, follow the [magic links guide](/docs/lifecycle/authenticate-users/passwordless/magic-links). In brief, you need to ensure that you have an email template configured in your FusionAuth tenant under <Breadcrumb>Email -> Template settings -> Passwordless login</Breadcrumb>  to send the user an email (the template was configured for you in the [Kickstart file of the repository](https://github.com/fusionauth/fusionauth-example-docker-compose/blob/main/light/kickstart/kickstart.json) you downloaded) and that you have passwordless login enabled in the <Breadcrumb>Security</Breadcrumb> tab of the application settings.
...
If you enter the email address of a user registered in the application and click the <InlineUIElement>Send</InlineUIElement> button, a login URL will be emailed to the registered email.
...
This final example shows how to ask a user for confirmation with their passkey instead of an OTP. It uses the [SimpleWebauthn TypeScript library](https://simplewebauthn.dev/docs).
