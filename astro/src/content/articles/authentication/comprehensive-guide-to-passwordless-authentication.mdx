...
- **Losing access to recovery email address**
...
- **Password sharing**: Employees in workplaces share passwords, causing security risks (including sabotage), and making auditing and non-repudiation impossible. For paid services like Netflix, sharing passwords also makes fraud between friends easy and fun.
...
If you need to revise asymmetric cryptography, see the [Wikipedia page](https://en.wikipedia.org/wiki/Public-key_cryptography). All you need to remember is that there are two keys, and whatever is encrypted with one key can be decrypted only with the other key.
...
Because the user has almost nothing to do in comparison to using passwords or multi-factor authentication, passkeys are said to be frictionless login. All the user will see is something like the screenshot below.

![WebAuthn demo](../../../../public/img/articles/authentication/comprehensive-guide-to-passwordless/webauthn.webp)

Test passkeys your self at https://webauthn.io
...
The browser itself is not the device, but needs access to a device that can generate and authenticate with public-key cryptography. The device might be built into the hardware and operating system, such as on Android and iOS phones. The device might be an external USB cryptography device, such as a Yubikey. The device might be a browser extension, such as Bitwarden. (Note this implies a user is able to choose different devices when authenticating on a site, leading to confusion and inability to log in if they use many different devices).

For ideal security, the private keys would never leave the hardware device. The browser would pass the challenge to the device, which would return the encrypted version and never exposing the key. This system existed long before WebAuth, and is called a hardware security module (HSM). The most famous examples of HSMs are the trusted platform module (TPM), which is required for upgrading to Windows 11, or cryptocurrency wallets, which prevent users accidentally exposing their Bitcoin keys.
...
All the synchronization works the same way, no matter the provider you use: the passkeys are encrypted with the user's master password, uploaded, stored, and downloaded, where they are decrypted with the user's master password.

#### So Passkeys Need A Password?

Now you might ask: so ultimately passkeys are dependent on passwords? What about "biometrics" I hear about?

Yes, ultimately the user needs to authenticate with their device, and a strong password is needed to encrypt all the passkeys. However, a local copy of the passkeys can be stored on the device that are encrypted not with the master password, but with something more convenient, like a short PIN (personal identification number), a fingerprint, or face photograph. And if you lose access to your master password, you lose access to all your passkeys.
...
- **Inconvenience**: Websites have onerous requirements to create passwords, that differ from site to site, and are often cryptographically not even secure. Users might also be asked to change their passwords frequently.
- **Forgetfulness**: Users forget their passwords, especially if they have multiple accounts or don't use certain accounts often.
- **Reuse**: Users use the same password on many sites. If one site is hacked, all others using the same password are in danger.
- **Losing access to recovery email address**
- **Guessable**: Weak passwords are easy for hackers to guess or brute force using dictionary attacks or rainbow tables.
- **Theft**: Even with a strong password, a user can be tricked into revealing a password on a fake website (phishing), by social engineering, physical theft, shoulder surfing, key logging, or an evil maid attack.
- **Breaches**: If hackers gain access to a company's website, all customers' passwords are in danger. This is worse if the website didn't hash the passwords, or hashed them with weak algorithms.
- **Password sharing**: Employees in workplaces share passwords, causing security risks, and making auditing and non-repudiation impossible. For paid services like Netflix, sharing passwords also makes fraud between friends easy.
...
<Aside type="caution">
If you don't run all the commands fast enough, your authentication request might time out. If any part of your JSON is incorrect, FA will give you the vague error message:
`{"fieldErrors":{},"generalErrors":[{"code":"[Exception]","message":"FusionAuth encountered an unexpected error. Please review the troubleshooting guide found in the documentation for assistance and the available support channels."}]}`. To find the problem, you will need to compare the JSON you are sending to the FA [API specification](https://fusionauth.io/docs/apis/webauthn#complete-a-webauthn-passkey-assertion), and to the expected [example JSON parameter](https://fusionauth.io/docs/lifecycle/authenticate-users/passwordless/webauthn-passkeys#completing-the-assertion), manually. There is a [bug](https://github.com/FusionAuth/fusionauth-issues/issues/3001) logged to fix the vague error message, which is internally a null pointer exception.
</Aside>
