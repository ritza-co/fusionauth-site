...
This article explains the alternatives to traditional username/password login, how they work technically, when to use them, and how difficult they are for both users and programmers. By the end of this article, you should be able to answer questions like:
...
- **Sharing**: Employees in workplaces share passwords, causing security risks (including sabotage), and making auditing and accountability impossible. For paid services like Netflix, sharing passwords also makes fraud between friends easy and fun.
...
Note the word "browser", not "user". The user doesn't do anything in this process, except consent to register or authenticate. The user doesn't make a password — the browser makes a passkey. So the authentication process is ultimately between a server and a device, not a server and a user.
...
Operating systems may not support passkeys. For instance, users of Linux and users of older versions of Windows that don't log in to an online account don't have passkey support. Below is the confusing screen you see if you try to use a passkey website on Ubuntu.
