...
For desktop apps, like Steam or Discord, you don't have to use passwords. All modern operating systems allow a desktop app to open a browser window and return a value to the app — so magic links and passkeys are usable here too.
...
Using passkeys requires [a free community license](/docs/get-started/core-concepts/plans-features). Some advanced features might require a paid license.
</Aside>
...
To configure magic links, follow the [guide](/docs/lifecycle/authenticate-users/passwordless/magic-links). In brief, you need to ensure that you have an email template configured in the FusionAuth UI under the **Email tab -> Template settings** of your Tenant  to send the user an email (which was done for you in the [Kickstart file of the repository](https://github.com/fusionauth/fusionauth-example-docker-compose/blob/main/light/kickstart/kickstart.json) you downloaded) and enable passwordless login in the **Security** tab of the application settings.

![Enable passwordless](/img/articles/authentication/comprehensive-guide-to-passwordless/turn-on-passwordless.png)

Email templates can be customized by navigating to **Customizations -> Email Templates**, so you can format your magic links and their accompanying message how you want.
...
If you enter an email address of a user registered in the application and click the <InlineUIElement>Send</InlineUIElement> button a login URL will be emailed to the registered email.

![Magic link email](/img/articles/authentication/comprehensive-guide-to-passwordless/magicEmail.png)

Clicking this link will log you in to FusionAuth and redirect you to the URL of the website set in the application under **OAuth tab -> Authorized redirect URLs**.
...
To configure MFA, follow the [guide](/docs/lifecycle/authenticate-users/multi-factor-authentication). In brief, you need to enable email settings and select the email template in your tenant **Multi-Factor** tab and set multi-factor <InlineField>On login policy</InlineField> to `Required` for your application.
...
After clicking the <InlineUIElement>Send a one-time code</InlineUIElement> button you'll receive a pin via email which you can enter as the <InlineField>Verification code</InlineField> and click the <InlineUIElement>Enable</InlineUIElement> button.
...
You will need to enable a self-service form by selecting a <InlineField>User Self-service</InlineField> form under the <Breadcrumb>Registration</Breadcrumb> tab of your application and then direct your existing users to the self-service account management page to create a passkey, which can be found on the <Breadcrumb>Application -> View</Breadcrumb> page in FusionAuth. For this example, the self-service page is http://localhost:9011/account/?client_id=e9fdb985-9173-4e01-9d73-ac2d60d1dc8e.
...
Now you, as a user, can browse to the [registration URL](http://localhost:9011/oauth2/register?client_id=e9fdb985-9173-4e01-9d73-ac2d60d1dc8e&response_type=code&redirect_uri=https%3A%2F%2Ffusionauth.io) and register with any email address and password you want. (For testing purposes, it's faster if you as the administrator disable verifying email addresses in the application settings).
...
![Reauthenticate](/img/articles/authentication/comprehensive-guide-to-passwordless/reauth.webp)
...
<Aside type="caution">
If you don't run all the commands fast enough, your authentication request might time out. If any part of your JSON is incorrect, FusionAuth will give you the error message:
`{"fieldErrors":{},"generalErrors":[{"code":"[Exception]","message":"FusionAuth encountered an unexpected error. Please review the troubleshooting guide found in the documentation for assistance and the available support channels."}]}`. To find the problem, you will need to compare the JSON you are sending to the FusionAuth [API specification](/docs/apis/webauthn#complete-a-webauthn-passkey-assertion), and to the expected [example JSON parameter](/docs/lifecycle/authenticate-users/passwordless/webauthn-passkeys#completing-the-assertion), manually.
</Aside>
...
But if you're using deep integration, you need to consider cost, feature set, ease of use, reliability, support, documentation, customization, and hosting possibilities. FusionAuth has detailed comparison articles on different gateways [here](/blog/category/compare/).
...
- **Scalability**: Can the gateway handle your number of users, including usage spikes at peak times?
...
- [FusionAuth documentation on passwordless authentication](/docs/lifecycle/authenticate-users/passwordless/)
- [FusionAuth gateway comparisons](/blog/category/compare)
- [FusionAuth gateway migration guides](/docs/lifecycle/migrate-users/general-migration)