...
- This article won't work until FA updates https://github.com/fusionauth/fusionauth-example-docker-compose to Ritza's latest version
*/}



This article discusses the dangers of storing OAuth tokens in the browser or client-side device – common for serverless apps like single-page apps (SPAs) and mobile apps – and how to upgrade to using an OAuth proxy server. An OAuth proxy is also known as the token handler pattern, or an OAuth backend for frontend (BFF).
...
SoundCloud had a single API that served web, Android, iOS, and desktop applications. The diagram below shows this.

![One API](../../../../public/img/articles/authentication/bff/bff1.png)

This design has a few problems:

- Each frontend (web, mobile, and desktop) has to convert the API's data into a format it can use to display content. The frontend also has to make multiple API calls and combine them to get all the data it needs. This is extra work for the user's device and extra bandwidth.
...
Each backend can now be considered part of the app. Features can be implemented freely by working on a backend and frontend pair, without affecting any other app.

## What Is BFF For Authentication?

BFFs were proposed for developer ease, not for security (authentication and authorization). After all, most web apps ten years ago had a server, whether it ran PHP, ASP, or Node.js, and most apps today still do. Even single-page apps (SPAs) that used AJAX (fetch calls) to dynamically update web pages still updated them by calling a server.
...
The use of a cookie as the authentication credential in the browser is crucial, as opposed to storing the credential in the browser's local or session storage. A cookie can be set as `HttpOnly` and `SameSite=Strict`, meaning that JavaScript cannot access the cookie, and the cookie won't be sent in a request to any server except the one that created it.
...
- The OAuth server redirects the browser back to the web service, with a code in the URL.
...
![Server to Dropbox](../../../../public/img/articles/authentication/bff/bff6.svg)
...
![BFF server with Oauth](../../../../public/img/articles/authentication/bff/bff7.svg)

Again, this design is identical to the earlier app design that had a traditional web server. The reason that this pattern is now called a BFF server is that it is designed exclusively to provide authorization services to the frontend. The real business service server is actually the resource server.
...
![BFF servers](../../../../public/img/articles/authentication/bff/bff8.svg)
...
- For a mobile or desktop app, there is less value because these apps might not store cookies like browsers do. However, the BFF can still return a short-lived authentication credential, so if the attacker steals the credential, they won't have long to act. If you don't use a BFF, once the attacker steals the refresh token, they might be able to impersonate a user for days.

Additionally, a BFF might expose fewer endpoints to the frontend, limiting the actions an attacker can take. The BFF might be able to check for unusual behavior that indicates an attack, and not forward the request to the business server (similarly to how banks check for credit card fraud).

Read more about [where to store tokens on different devices](https://fusionauth.io/articles/oauth/oauth-token-storage).

As mentioned earlier, in the event that there is malicious JavaScript in the frontend itself, even if you use a BFF, then while an attacker may not be able to steal the refresh token, they can still perform any action they want from the user's device while impersonating the user.
...
Let's look at some code to create a BFF for a currently serverless app. First you'll see how the app runs and login works, then you'll learn how the code works.

Clone or download the repository from https://github.com/ritza-co/fusionauth-example-hostedbackend. 
...
In a new terminal, run the commands below to start the Node.js web app.
...
The BFF in `app.js` is the tiniest Express server possible. 
...
- With the existing React SDK for FusionAuth. FusionAuth provides a tutorial and example repository that show [how to integrate authentication into your app](https://fusionauth.io/docs/quickstarts/quickstart-javascript-react-web).
...
{/* Tatenda, you can remove the paragraph below if Dan explains how to fix, I'm still waiting to hear back from him: https://github.com/FusionAuth/fusionauth-issues/issues/3054 */}

Browse to the Applications tab of FusionAuth at http://localhost:9011/admin/application. For the `Example App`, click <Breadcrumb>Select -> Edit</Breadcrumb>. On the <InlineUIElement>Scopes tab</InlineUIElement>, change <InlineUIElement>Scope handling policy</InlineUIElement> to <InlineUIElement>Compatibility</InlineUIElement>. If you leave this value on <InlineUIElement>Strict</InlineUIElement>, the user's email won't be returned.
...
![Id token](../../../../public/img/articles/authentication/bff/idToken.png)
...
In a full single-page app, you'll probably use a client-side routing framework. But this example checks for only two cases: Either it is handling a callback from the authentication server and checks the challenge code matches, or it displays the login link on the page for any other route. In either case, if there is a logged in user Id cookie present, the user's email address is displayed on the screen.

All the logging link does is redirect the user to the FusionAuth hosted backend API endpoint.

When making the request, you need to specify the `openid` [scope](https://fusionauth.io/docs/lifecycle/authenticate-users/oauth/scopes), or the user's email address won't be returned in the `app.idt` token.
...
If you don't want to use FusionAuth and don't want to write your own BFF, there are open-source projects that will handle cookies and tokens for you, like [OAuth2 Proxy](https://oauth2-proxy.github.io/oauth2-proxy), which is written in Go. You still need a server somewhere to host it, though.

The following authentication gateways offer OAuth proxies:
...
- Duende offers a [ASP.NET OAuth Proxy package](https://blog.duendesoftware.com/posts/20210326_bff/) you can host, as part of its IdentityServer.
...
- Always use a server for apps that use OAuth, even mobile and desktop apps if possible, so tokens are never sent to the browser. If your apps are currently serverless, add an OAuth proxy to make them safer.
- The easiest way to add an OAuth proxy is with the FusionAuth hosted backend, the Go Auth2 Proxy, or building on the Passport.js Node sample in this article. (OAuth wrappers exist for other languages, too, so you won't have to write your own protocol code.)
- The greatest danger to your app is malicious dependencies. Be careful which packages you build with.
- For dangerous actions, like buying, selling, or accessing private information, consider using [multifactor authentication](https://fusionauth.io/docs/lifecycle/authenticate-users/multi-factor-authentication) in addition to any other security practices you have in place.
...
- [What OAuth tokens are used for](https://fusionauth.io/blog/what-happens-after-oauth-authentication-event)
...
- [FusionAuth OAuth scopes](https://fusionauth.io/docs/lifecycle/authenticate-users/oauth/scopes)
