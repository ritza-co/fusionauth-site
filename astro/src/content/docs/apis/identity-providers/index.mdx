...
You can test Identity Provider connections using the [Test API](/docs/apis/identity-providers/test).
...
<TryInExplorer method="GET" path="/api/identity-provider" tag="identity-providers" />

## Retrieve All Identity Providers

### Request

Retrieve all of the configured Identity Providers.
...
Search for Identity Providers using the `GET` method with query parameters.
...
Look up the Identity Provider that manages a given email domain or address.
...
<APIBlock>
  ...
  <APIField name="tenantId" type="UUID" optional since="1.62.0">
