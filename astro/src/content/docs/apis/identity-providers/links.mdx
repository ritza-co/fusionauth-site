...
Link a User using the current request body format, or using the deprecated request body format.
...
#### Link a User (deprecated request body)
...
<API method="POST" uri="/api/identity-provider/link" authentication={["api-key"]} title="Link a User (deprecated request body)"/>
...
<StandardPostResponseCodes conflict_detection never_search_error webhook_event />
...
Complete a pending Link using the current request body format, or using the deprecated request body format.
...
#### Complete a pending Link (deprecated request body)
...
<API method="POST" uri="/api/identity-provider/link" authentication={["api-key"]} title="Complete a pending Link (deprecated request body)"/>
...
Retrieve a single link, all links for a specific user by identity provider, or all links for a user.
...
Retrieve a pending Link by its Id.
...
Unlink a User from a 3rd party identity provider by providing the identity provider and user Ids.
