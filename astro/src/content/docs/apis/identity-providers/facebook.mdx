...
The Facebook identity provider type will use the Facebook OAuth login API. It will provide a <Breadcrumb>Login with Facebook</Breadcrumb> button on FusionAuth's login page that will leverage the Facebook login pop-up dialog. Additionally, this identity provider type will call Facebook's Graph API to load additional details about the user and store them in FusionAuth.
...
Create a Facebook Identity Provider with either a randomly generated Id or one you provide on the URI.
...
Retrieve the Facebook Identity Provider by its Id or by type.
...
Update the Facebook Identity Provider by sending the fields you want to change.
...
<StandardPutResponseCodes conflict_detection />
...
Delete the Facebook Identity Provider by its Id or by type.
...
Complete the Facebook login by providing the access token received from Facebook.
