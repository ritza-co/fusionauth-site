...
Leave off the Id to retrieve all of the Consents, or include it on the URI to retrieve a single Consent.

<API method="GET" uri="/api/consent" authentication={["api-key"]} title="Retrieve all of the Consents" headingLevel="h3"/>

<API method="GET" uri="/api/consent/{consentId}" authentication={["api-key"]} title="Retrieve a Consent by Id" headingLevel="h3"/>
