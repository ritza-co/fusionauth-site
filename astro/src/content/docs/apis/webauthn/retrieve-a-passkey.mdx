...
Retrieve a single Passkey by Id, or all Passkeys belonging to a User.

<API method="GET" uri="/api/webauthn/{id}" authentication={["api-key"]} title="Retrieve a single Passkey by Id" headingLevel="h3"/>
...
<API method="GET" uri="/api/webauthn?userId={userId}" authentication={["api-key"]} title="Retrieve all Passkeys belonging to a User" headingLevel="h3"/>
