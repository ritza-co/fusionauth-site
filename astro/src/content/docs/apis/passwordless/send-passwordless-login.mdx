...
<TryInExplorer method="POST" path="/api/passwordless/send" tag="passwordless" />

## Request

Send the provided code via email or SMS, or generate a code and send a login link via email.
...
<API method="POST" uri="/api/passwordless/send" authentication={["none"]} title="Generate a code and send a login link via email" headingLevel="h3"/>
