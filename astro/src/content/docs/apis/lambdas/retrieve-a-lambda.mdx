...
<TryInExplorer method="GET" path="/api/lambda" tag="lambdas" />

## Request

Retrieve all of the Lambdas, optionally filtered by type, or a single Lambda by Id.

<API method="GET" uri="/api/lambda" authentication={["global-api-key"]} title="Retrieve all of the Lambdas" headingLevel="h3"/>
...
<API method="GET" uri="/api/lambda/{lambdaId}" authentication={["global-api-key"]} title="Retrieve a Lambda by Id" headingLevel="h3"/>
