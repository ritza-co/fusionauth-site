...
<sup>*</sup> You will need to use the Complete API if you want to use `FormField` verification or if you don't want to use FusionAuth's `ClickableLink` page.
...
<TryInExplorer method="POST" path="/api/identity/verify/start" tag="identity" />
...
Start identity verification by providing the login identifier to verify.
...
Send the verification code for a previously started verification.
...
Complete identity verification by providing the verification Id and the code.
