...
<TryInExplorer method="POST" path="/oauth2/device_authorize" tag="oauth2" />
...
<API method="POST" uri="/oauth2/device/user-code" title="Validate and retrieve a user_code using client credentials" headingLevel="h3"/>
...
<API method="POST" uri="/oauth2/device/user-code" authentication={["api-key"]} title="Validate and retrieve a user_code using an API key" headingLevel="h3"/>
