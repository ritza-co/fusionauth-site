...
Calling this endpoint does not revoke refresh tokens or JWTs. You can revoke the former by using the [Revoke Refresh Tokens API](/docs/apis/jwt/revoke-refresh-tokens). To revoke JWTs, [implement a revocation strategy](/articles/tokens/revoking-jwts).
...
Log the user out with a `GET` request using query parameters, or a `POST` request using a form body.
