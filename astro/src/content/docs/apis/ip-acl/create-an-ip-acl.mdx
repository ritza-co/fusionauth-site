---
title: Create an IP ACL
description: API documentation for the FusionAuth Create an IP ACL API.
order: 100
---
import API from 'src/components/api/API.astro';
import APIBlock from 'src/components/api/APIBlock.astro';
import APIField from 'src/components/api/APIField.astro';
import IpAclRequestBody from 'src/content/docs/apis/_ip-acl-request-body.mdx';
import IpAclResponseBody from 'src/content/docs/apis/_ip-acl-response-body.mdx';
import StandardPostResponseCodes from 'src/content/docs/apis/_standard-post-response-codes.astro';
import TryInExplorer from 'src/components/api/TryInExplorer.astro';

This API is used to create a new IP ACL.

<TryInExplorer method="POST" path="/api/ip-acl" tag="ip-access-control-lists" />

## Request

Create an IP ACL with either a randomly generated Id or one you provide on the URI.

<API method="POST" uri="/api/ip-acl" authentication={["global-api-key"]} title="Create an IP ACL with a randomly generated Id"/>

<API method="POST" uri="/api/ip-acl/{ipAccessControlListId}" authentication={["global-api-key"]} title="Create an IP ACL with the provided unique Id"/>

### Request Parameters

<APIBlock>
  <APIField name="ipAccessControlListId" type="UUID" optional defaults="secure random UUID">
    The Id to use for the new IP ACL. If not specified a secure random UUID will be generated.
  </APIField>
</APIBlock>

<IpAclRequestBody />

## Response

The response for this API contains the IP ACL that was created.

<StandardPostResponseCodes never_search_error />

<IpAclResponseBody />
