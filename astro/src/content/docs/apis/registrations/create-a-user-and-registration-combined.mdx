...
This API is used to create a new User and a User Registration in a single request. This is useful if for example you have a main website that Users create their account on initially. The User is technically creating their global User object and a User Registration for that website (i.e. that Application). In this case, you will want to create the User and the User Registration in a single step. This is the API to use for that. You can optionally still provide an Id for the new User on the URI. If you don't specify an Id for the User, FusionAuth will create one for you.

If the User already exists and you only need to register them for a new Application, use [Create a User Registration (for an existing user)](/docs/apis/registrations/create-a-user-registration-for-an-existing-user) instead.

<TryInExplorer method="POST" path="/api/user/registration/{userId}" />

## Request

Create the User and their Registration for an Application in one call.
