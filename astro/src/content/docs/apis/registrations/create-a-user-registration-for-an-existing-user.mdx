...
This API requires a User that already exists. Create one first with the [Create a User](/docs/apis/users/create) API, or if you need to create the User and the Registration together, use [Create a User and Registration (combined)](/docs/apis/registrations/create-a-user-and-registration-combined) instead.

<TryInExplorer method="POST" path="/api/user/registration/{userId}" />

## Request

Register the existing User for an Application by providing the User's Id on the URI and the Application Id in the request body.
