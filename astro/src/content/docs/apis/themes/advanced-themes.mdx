...
<TryInExplorer method="POST" path="/api/theme" tag="themes" />
...
This API supports two ways to create an Advanced Theme, either as a brand new Theme or by copying an existing Theme's settings.

#### Create a New Advanced Theme
...
#### Copy an Existing Advanced Theme

<API method="POST" uri="/api/theme" authentication={["global-api-key"]} title="Create a Theme from an existing Theme with a randomly generated Id"/>

<API method="POST" uri="/api/theme/{themeId}" authentication={["global-api-key"]} title="Create a Theme from an existing Theme with the provided unique Id"/>
...
Retrieve all of the Themes, or provide an Id to retrieve a single Theme.
...
Update the Theme by sending the fields you want to change.
