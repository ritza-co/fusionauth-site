...
<TryInExplorer method="GET" path="/api/tenant-manager" tag="tenant-manager" />
...
<GenericUpdateExplanationFragment capitalized_object_name="Tenant Manager Configuration" />

### Request

Update the Tenant Manager configuration by sending the fields you want to change.
...
Create a Tenant Manager identity provider type configuration for the given type.
...
Update the Tenant Manager identity provider type configuration by its type.
...
Delete the Tenant Manager identity provider type configuration by its type.
