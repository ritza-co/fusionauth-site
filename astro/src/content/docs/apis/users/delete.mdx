...
Soft deleted users are marked as inactive but not deleted from FusionAuth. Deactivated users have their data retained but they are unable to authenticate. Users who have been deactivated can be reactivated; see [Reactivate a User](/docs/apis/users/reactivate) for more.
...
<TryInExplorer method="DELETE" path="/api/user/{userId}" />

## Request

Choose a soft delete to deactivate the User, or add `hardDelete=true` to permanently remove their data.
