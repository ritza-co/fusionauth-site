...
The Login API is used to authenticate a user in FusionAuth. The user must already exist, see [Create a User](/docs/apis/users/create) if you don't have one yet.
...
<APIBlock>
  <APIField name="fusionauth.trust" type="String" optional>
    The Multi-Factor Trust identifier returned by the [Complete Multi-Factor Authentication](/docs/apis/login/complete-mfa) API response. This value may be provided to bypass the Multi-Factor challenge when a User has Multi-Factor enabled. When this cookie exists on the request it will take precedence over the **twoFactorTrustId** if provided in the request body.
  </APIField>
</APIBlock>
