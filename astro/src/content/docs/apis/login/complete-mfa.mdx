...
The Multi-Factor Login API is used to complete the authentication process when a `242` status code is returned by the [Login](/docs/apis/login/authenticate) API or as the last step of a [Step Up Procedure](/docs/lifecycle/authenticate-users/multi-factor-authentication#step-up-auth).
