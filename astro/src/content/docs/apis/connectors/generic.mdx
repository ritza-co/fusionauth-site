...
<TryInExplorer method="POST" path="/api/connector" tag="connectors" />

## Create the Generic Connector

### Request

Create a Generic Connector with either a randomly generated Id or one you provide on the URI.

<API method="POST" uri="/api/connector" authentication={["global-api-key"]} title="Create a Generic Connector with a randomly generated Id."/>

<API method="POST" uri="/api/connector/{connectorId}" authentication={["global-api-key"]} title="Create a Generic Connector with the provided unique Id."/>
...
Retrieve the Generic Connector by its Id.
...
Update the Generic Connector by sending the fields you want to change.
...
Delete the Generic Connector by its Id.
