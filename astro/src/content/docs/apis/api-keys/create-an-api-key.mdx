...
A key with key manager permissions can not be created using this API. Only through admin UI or [Kickstart](/docs/get-started/download-and-install/development/kickstart) can you create such a key.

<TryInExplorer method="POST" path="/api/api-key" tag="api-keys" />

## Request

This API supports two ways to create a Key, either as a brand new Key or by cloning an existing Key's permissions.

### Create a New Key

Use a randomly generated Id in most cases. Supply your own Id if you need a predictable identifier for the key.
...
To clone an existing key's permissions into a new key, use one of the following instead.
...
The response for this API contains the Key that was created. The **key** field is the value you'll supply in the `Authorization` header for subsequent requests.
