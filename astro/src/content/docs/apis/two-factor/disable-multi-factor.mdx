...
If using message based delivery, you may [Send a Multi-Factor Code When Disabling MFA](/docs/apis/two-factor/send-a-multi-factor-code-when-disabling-mfa) to deliver a code to the User. The User will then provide this code as input.

If a recovery code is provided, all methods will be removed.

<TryInExplorer method="DELETE" path="/api/user/two-factor/{userId}" />

## Request

Disable a multi-factor method by User Id or by JWT, sending the verification or recovery code as query parameters or in a JSON body.
