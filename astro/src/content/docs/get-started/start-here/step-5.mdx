...
When using FusionAuth as an OAuth identity provider, a user logs in using what is known as the **Authorization Code grant**. This leverages FusionAuth's hosted login pages to authenticate the user and provide an access token to the application.
...
<LocalCode src="example-get-started/src/sdk.ts.snippet.logInUser.ts" lang="typescript" />
...
<LocalCode src="example-get-started/src/sdk.ts.snippet.getUser.ts" lang="typescript" />
...
<LocalCode src="example-get-started/src/sdk.ts.snippet.handleJWTException.ts" lang="typescript" />
