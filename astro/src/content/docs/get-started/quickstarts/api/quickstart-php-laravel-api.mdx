...
<Aside type="caution">
The FusionAuth PHP JWT library does not yet support Laravel version 13. You need to use Laravel version 12 or lower.
</Aside>
...
```shell-session
composer create-project laravel/laravel your-application "^12.0"
cd your-application
php artisan install:api
php artisan serve --port=3000
```

You can browse to the default Laravel [app homepage](http://localhost:3000).

### Add Security

Stop the server and include FusionAuth's library to add JWT support to the authentication system:
...
When installing Laravel, it will automatically make a migration to create a `users` table in the database. You'll have to edit the file `database/migrations/0001_01_01_000000_create_users_table` to change the default behavior and use UUID for the primary key, remove the need for a password and the remember token.

<LocalCode src="quickstart-php-laravel-api/complete-application/database/migrations/0001_01_01_000000_create_users_table.php" lang="php"/>
...
By default, Laravel allows only JWTs that correspond to users in your database, but one of the greatest benefits of using FusionAuth is to have a single source of truth of user management. So, you want your API to automatically provision new users when it receives a trusted JWT from FusionAuth.
...
<LocalCode src="quickstart-php-laravel-api/complete-application/app/FusionAuth/Providers/FusionAuthEloquentUserProvider.php" lang="php"/>
...
<LocalCode src="quickstart-php-laravel-api/complete-application/app/FusionAuth/FusionAuthJwtGuard.php" lang="php"/>

Let Laravel know about these two classes by editing `config/auth.php`.

First, change the `driver` for the `web` Guard to `jwt`:

<LocalCode src="quickstart-php-laravel-api/auth.snippet.guards.php" lang="php"/>

Now, change the default `provider` to `fusionauth_eloquent`:

<LocalCode src="quickstart-php-laravel-api/auth.snippet.providers.php" lang="php"/>
...
<LocalCode src="quickstart-php-laravel-api/complete-application/app/FusionAuth/Providers/FusionAuthServiceProvider.php" lang="php"/>
...
<LocalCode src="quickstart-php-laravel-api/complete-application/app/FusionAuth/Claims/Audience.php" lang="php"/>
...
<LocalCode src="quickstart-php-laravel-api/complete-application/app/FusionAuth/Claims/Issuer.php" lang="php"/>
...
Start by adding the environment variables below to your `.env` file.

<LocalCode src="quickstart-php-laravel-api/fusionauth.snippet.fusionauth.env" lang="ini"/>

Add the FusionAuth provider to the `bootstrap/providers.php` providers array:

<LocalCode src="quickstart-php-laravel-api/complete-application/bootstrap/providers.php" lang="php"/>
