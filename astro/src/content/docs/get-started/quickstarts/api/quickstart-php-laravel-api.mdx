...
* [PHP](https://www.php.net/manual/en/install.php) 8.3 or later
...
Both endpoints will be protected such that a valid JSON web token (JWT) will be required as a `Bearer` token in the `Authorization` header in order to be accessed. Additionally, the JWT must have a `roles` claim containing the appropriate role to use the endpoint.
...
A new Laravel application only registers web routes. This API validates FusionAuth tokens itself, so it does not need Laravel Sanctum or `php artisan install:api`. Instead, create an empty `routes/api.php` file, which you will fill in later:

```shell-session
touch routes/api.php
```

Then register it in `bootstrap/app.php`. This also tells Laravel to render errors for `/api/*` requests as JSON rather than HTML:
...
### Add the Signing Key Set

FusionAuth publishes the public keys that verify its tokens at a JSON Web Key Set (JWKS) endpoint. Create a class that fetches those keys and caches them, so that the API does not call FusionAuth on every request:

```shell-session
mkdir -p app/Auth
```

Create `app/Auth/FusionAuthKeySet.php`:

<LocalCode src="quickstart-php-laravel-api/complete-application/app/Auth/FusionAuthKeySet.php" lang="php"/>

The key set is handed to the Firebase JWT library, which asks it for the key named by the token's `kid` header. A few details are worth calling out:

- When a token names a key Id that is not in the cache, the key set fetches the JWKS again, so a rotated signing key is picked up right away instead of failing until the cache expires. The refresh happens at most once per minute, so a flood of tokens with made-up key Ids cannot turn your API into a request generator against FusionAuth.
- Every key is bound to the single algorithm you configure (`RS256` below), whatever the JWKS entry declares. The library then refuses any token whose header names a different algorithm.
- Fetching the JWKS uses short timeouts, and a failure to reach or parse it is a server error. It is never reported to the client as an invalid token.

### Add the JWT Middleware

Create a middleware to validate JWTs from FusionAuth. It reads the token from the `Authorization` header, verifies the signature using the key set, checks the claims, and provisions a user record on the first request:
...
The middleware only reads the token from the `Authorization` header. Browsers attach cookies to cross-site requests automatically, so accepting the token from a cookie would expose every endpoint to cross-site request forgery unless you also added CSRF protection. Sending the token as a `Bearer` header is the standard for an API resource server and needs no such protection.

The Firebase library verifies the signature and the `exp`, `nbf` and `iat` claims when they are present, but it does not fail when they are absent. The middleware therefore requires the claims the API relies on: `sub` must be a UUID (the FusionAuth user Id), `exp` must be present, `iss` must be your FusionAuth URL, and `aud` must contain your application's client Id. FusionAuth issues `aud` as a string, or as an array when the token was requested for additional resources, so both forms are accepted.

Any failure to validate the token results in a `401` response with a generic body. The reason is logged at `info` level, since invalid tokens are routine, and is never sent to the client.

### Update Routes

Fill in `routes/api.php` to use the middleware and add both `/api/make-change` and `/api/panic` endpoints:
...
Make a request to `/api/make-change` with a query parameter `total=5.12` and pass the `token` as a `Bearer` token in the `Authorization` header.
...
You were authorized, success! You can try making the request without the header, with the token in an `app.at` cookie instead, or with a different string rather than a valid token, and see that you are denied access with a `401` response.
...
```shell-session
curl --header 'Authorization: Bearer YOUR_TOKEN' --location 'http://localhost:3000/api/make-change?total=3.24'
```
...
```shell-session
curl --header 'Authorization: Bearer YOUR_TOKEN' -i --request POST -H "Accept: application/json" 'http://localhost:3000/api/panic'
```
