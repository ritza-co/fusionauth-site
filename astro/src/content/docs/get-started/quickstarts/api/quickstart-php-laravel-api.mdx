...
laravel new your-application --database=sqlite --no-node --no-boost # if this fails, try the full path: $COMPOSER_HOME/vendor/bin/laravel new your-application --database=sqlite --no-node --no-boost
...
Initialize the Laravel application using the following commands:
