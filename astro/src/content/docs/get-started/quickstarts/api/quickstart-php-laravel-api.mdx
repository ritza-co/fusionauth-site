...
You were authorized, success! You can try making the request without the token or with a different string rather than a valid token, and see that you are denied access.
...
```shell-session
curl -H 'Authorization: Bearer YOUR_TOKEN' --location 'http://localhost:3000/api/make-change?total=3.24'
```
...
```shell-session
curl -H 'Authorization: Bearer YOUR_TOKEN' -i --request POST -H "Accept: application/json" 'http://localhost:3000/api/panic'
```
