...
```shell-session
# docker run --name app --network host -v vendor:/app/vendor -v .:/app -w /app -it --rm composer:2.10 bash
#composer global require laravel/installer:^5.32
#laravel new your-application ^12.0 --database=sqlite --no-node --no-boost

composer create-project laravel/laravel your-application "^12.0"

# if command above fails, try the full path: $COMPOSER_HOME/vendor/bin/laravel new your-application ^12.0   --database=sqlite --no-node --no-boost
# choose `no` for a starter kit, and then Blade
...
<LocalCode src="quickstart-php-laravel-api/app.snippet.providers.php" lang="php"/>
