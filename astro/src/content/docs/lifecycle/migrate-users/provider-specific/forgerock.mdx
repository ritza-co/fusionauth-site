...
<Aside type="note">
This guide was tested against {migration_source_name} 7.4. You can find the {migration_source_name} Cloud Development Kit [here](https://github.com/ForgeRock/forgeops).
</Aside>
...
* With {migration_source_name}, Clients are what your users can log in to. FusionAuth refers to these as [Applications](/docs/get-started/core-concepts/types/applications). In both cases, you can use either OAuth/OIDC or SAML to do so.
...
<Aside type="note">
By default, {migration_source_name} has disabled the exporting of the password hash through the Rest API. To enable, select <Breadcrumb>Identity Management Native Console -> Configure -> Managed Object -> User</Breadcrumb>. Find the 'password' field. Select Edit. Then, select the Privacy & <Breadcrumb>Encryption</Breadcrumb> tab. Next, set Private to false. You may wish to re-enable after the export.
...
<LocalCode lang="ruby" src="import-scripts/forgerock/forgerock-export.rb" title="Forgerock Export Script" />
