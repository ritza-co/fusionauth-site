...
This document will help you migrate from Keycloak to FusionAuth.

If you are looking to compare FusionAuth and Keycloak, [this document may help](/blog/keycloak-fusionauth-comparison).
...
![The Keycloak realm screen.](/img/docs/lifecycle/migrate-users/provider-specific/keycloak-tenant-screen.png)
...
* With Keycloak, Clients are what your users can log in to. FusionAuth refers to these as [Applications](/docs/get-started/core-concepts/types/applications). In both cases, you can use either OAuth/OIDC or SAML to do so.
* Realms are a high level construct which groups other entities such as users and clients together. FusionAuth calls these [Tenants](/docs/get-started/core-concepts/types/tenants). FusionAuth supports a multi-tenant configuration by default.
* For Keycloak, [Roles](https://www.keycloak.org/docs/latest/server_admin/#core-concepts-and-terms) provide information about what your users can do in your custom or off the shelf applications. Roles can be associated with other roles. FusionAuth has [Roles](/docs/get-started/core-concepts/types/roles) and they are defined on an Application by Application basis and cannot be composed.
...
<LocalCode lang="sql" src="import-scripts/keycloak/keycloak-export.sql" title="Keycloak Export MySQL SQL" />
...
<LocalCode lang="sql" src="import-scripts/keycloak/keycloak-export-postgres.sql" title="Keycloak Export Postgres SQL" />
...
The FusionAuth team has written a Keycloak compatible password hashing plugin. The **encryptionScheme** for this plugin is `salted-pbkdf2-hmac-sha256-512`. This plugin ships with versions of FusionAuth 1.34 and above. It is called the `Salted PBKDF2 with SHA-256 with 512-bit derived key` plugin.
