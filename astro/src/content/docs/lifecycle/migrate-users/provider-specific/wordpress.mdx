...
<LocalCode lang="java" src="contrib/Password Hashing Plugins/src/main/java/com/mycompany/fusionauth/plugins/ExampleWordPressPhpassPasswordEncryptor.java" />
...
- Stores all WordPress user details that don't map to FusionAuth in the **data** field. If there are details you want to include or exclude, alter the `dataToIgnore` list at the top of the file.
...
If the migration script ran successfully, you should be able to log in to the `WordpressTestImportApp` application with one of the imported users. In the [FusionAuth admin UI](http://localhost:9011/admin), navigate to <Breadcrumb>Applications -> WordpressTestImportApp</Breadcrumb>. Click <IconButton icon="view" /> next to the application and note the **OAuth IdP login URL**.
