...
![Diagram showing external websites using FusionAuth.](/img/docs/cloud/github_actions/get-started-run-in-the-cloud-prerequisites.png)
...
![Diagram showing how GitHub Actions are created, triggered, and run inside GitHub.](/img/docs/cloud/github_actions/get-started-run-in-the-cloud-some-definitions-and-goals.png)

## Minimal Example Using GitHub Actions
...
![A CI/CD diagram showing how to use GitHub actions with FusionAuth.](/img/docs/cloud/github_actions/get-started-run-in-the-cloud-github-actions.png)

In GitHub, click <Breadcrumb>Fork</Breadcrumb> and add the repository to your GitHub account. Then run `git clone https://github.com/<your_username>/fusionauth-example-github-actions.git` in a terminal to download it. Remember to replace `<your_username>` with your GitHub username.
...
<Aside type="note">To learn how FusionAuth runs in Docker, read [Install FusionAuth with Docker](/docs/get-started/download-and-install/docker). To learn how Express connects to FusionAuth, please read this [QuickStart](/docs/get-started/quickstarts/web/express).</Aside>
...
FusionAuth will start, and you will be able to log in at `http://localhost:9011/admin` with username `admin@example.com` and password `password`. In <Breadcrumb>Users</Breadcrumb>, [add a new user](/docs/get-started/core-concepts/types/users) by clicking the green <Breadcrumb>+</Breadcrumb> button.
...
For more details on upgrading FusionAuth, including non-silent upgrades, please read this [article](/docs/get-started/download-and-install/docker).

<Aside type="note">
  FusionAuth also has a paid version that [runs in the cloud](/docs/cloud/). If you want to avoid the hassle of managing, backing up, and upgrading FusionAuth yourself, this might be the solution for you. Tests in this guide will still work after the version of FusionAuth has been upgraded.
</Aside>
...
This will open the playwright window. Click on the <Breadcrumb>Run</Breadcrumb> button next to the test name to execute the test. You can see that Playwright runs a browser, clicks buttons, and fills in forms.

![Playwright test UI.](/img/docs/cloud/github_actions/playwright.png)

## Write a Test for Playwright

The code for the test Playwright runs is in the file `app/tests/test.spec.js`.

<LocalCode src="example-github-actions/app/tests/test.spec.js" lang="js" />
...
<Aside type="note">
  By default, GitHub prevents action workflows from running on forked repositories. To grant permission for the workflow to execute, navigate to the <Breadcrumb>Actions</Breadcrumb> tab on your forked repository and click on the <Breadcrumb>I understand my workflows, go ahead and enable them</Breadcrumb> button on the repository.
</Aside>
...
You can browse to the <Breadcrumb>Actions</Breadcrumb> tab in your GitHub repository and then select <Breadcrumb>Test FusionAuth login</Breadcrumb> on the left sidebar to see the workflow run. The most recent workflow run will appear at the top of the list and you can see further details by clicking on the title of the workflow run items. Clicking on the <Breadcrumb>run-tests</Breadcrumb> job item will reveal further details of the workflow execution steps.

![Passed GitHub action test.](/img/docs/cloud/github_actions/passedtest.png)
...
![Failed GitHub action test.](/img/docs/cloud/github_actions/failedtest.png)
...
<LocalCode src="example-github-actions/.github/workflows/test.yaml" lang="yaml" />
...
* Download the repository of source code to the server and compile it there.
* Compile the executable in the action and download it on the server.
* Build your app into a Docker image, upload the image to DockerHub, and download and run it on the server.
...
![Storing GitHub action secrets.](/img/docs/cloud/github_actions/secrets.png)
...
- [FusionAuth Express QuickStart](/docs/get-started/quickstarts/web/express)
- [FusionAuth upgrade guide](/docs/get-started/download-and-install/docker)
- [FusionAuth cloud version](/docs/cloud/)
