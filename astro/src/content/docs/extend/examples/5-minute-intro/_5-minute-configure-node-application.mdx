...
<LocalCode lang="ini" src="example-5-minute-guide/.env.sample" title="The Contents Of the .env.sample File" />
...
<If condition={props.installmethod === 'sandbox'}>Change the `BASE_URL` value to the sandbox URL: `https://sandbox.fusionauth.io`</If>

<If condition={props.installmethod !== 'sandbox'}>If you are not running locally, change the `BASE_URL` to the correct value.</If>
...
<LocalCode title="The Contents Of the package.json File" src="example-5-minute-guide/package.json" />
...
<LocalCode lang="javascript" src="example-5-minute-guide/routes/index.js" title="The Node.js Complete Express Router" />
