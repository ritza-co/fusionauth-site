...
You need to have Node.js installed. This document has been tested with versions 14 and 18, but should work with other versions.

```shell-session title="Testing the Node.js Installation"
node -v
```

```shell-session title="Result Of Testing the Node.js Installation"
v18.10.0
```
...
```shell-session title="Testing the Docker Installation"
docker -v
```

```shell-session title="Result Of Testing the Docker Installation"
...
<LocalCode lang="shell" src="containers/docker/fusionauth/.env" title="The .env file" />
