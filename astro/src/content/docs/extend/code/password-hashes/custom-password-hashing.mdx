...
*Password Encryptor*
<LocalCode src="example-password-encryptor/src/main/java/com/mycompany/fusionauth/plugins/ExamplePBDKF2HMACSHA1PasswordEncryptor.java" lang="java" />
...
<LocalCode src="example-password-encryptor/src/main/java/com/mycompany/fusionauth/plugins/guice/MyExampleFusionAuthPluginModule.java" lang="java" />
...
<LocalCode title="Unit Test" src="example-password-encryptor/src/test/java/com/mycompany/fusionauth/plugins/ExamplePBDKF2HMACSHA1PasswordEncryptorTest.java" lang="java" />
...
However, the hashing scheme is associated with each account and cannot be queried by the [User Search API](/docs/apis/users/search). You can view it for an individual user by navigating to <Breadcrumb>Users -> Your User -> Manage</Breadcrumb> and choosing the <Breadcrumb>Source</Breadcrumb> tab, but you can't query this value for all users.

If you have a plan which includes support and need to know which users are using a particular password hashing scheme, [open a support ticket](https://account.fusionauth.io/account/support) so that the team can assist.
