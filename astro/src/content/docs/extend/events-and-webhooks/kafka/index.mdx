...
![Kafka Configuration](/img/docs/extend/events-and-webhooks/kafka/kafka.png)
...
Specify a topic that you've already created in your Kafka cluster and press <Breadcrumb>Send test event</Breadcrumb> to make sure that the connection is working as expected. After seeing that it succeeded, don't forget to press <Breadcrumb>Save</Breadcrumb> in the top right to turn on the Kafka integration.
...
<LocalCode title="Example docker-compose.yml with Kafka" src="example-docker-compose/kafka/docker-compose.yml" lang="yaml" />
