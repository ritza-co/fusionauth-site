...
<Aside type="note">
  Permify uses the term entities to refer to anything that can have a relation in a Permify authorization schema. This is distinct from [FusionAuth entities](/docs/get-started/core-concepts/types/entity-management), which has no formal schema and is instead managed by code.
...
You can run Permify the open-source project on your own for free, forever. For more information, see the [Permify documentation](/permify-docs/).
...
To install Permify, please refer to the [Permify deployment guides](/permify-docs/setting-up/installation/intro). These also offer guidance on system requirements and how to deploy the software into Kubernetes and other environments.
...
<LocalCode lang="yaml" src="example-fine-grained-authorization/docker-compose.snippet.permifyDockerCompose.yml" title="Permify docker compose section" />

Review the Permify configuration reference for [supported configuration options](/permify-docs/setting-up/configuration).

### Set Up The Permify Schema

The following schema defines the relationships between entities in the example banking scenario:

<LocalCode lang="typescript" src="example-fine-grained-authorization/permify-setup/authmodel.txt" title="FGA schema" />
...
<LocalCode lang="typescript" src="example-fine-grained-authorization/permify-setup/src/schema.ts" title="FGA schema load script" />
...
<LocalCode lang="typescript" src="example-fine-grained-authorization/permify-setup/src/loaddata.snippet.loadRelationships.ts" title="Loading relationships" />
...
<LocalCode lang="typescript" src="example-fine-grained-authorization/permify-setup/src/loaddata.snippet.loadAttributes.ts" title="Loading attributes" />
...
<LocalCode lang="typescript" src="example-fine-grained-authorization/app/src/index.snippet.checkingPermission.ts" title="Checking permissions" />
...
* [A modeling authorization guide](/permify-docs/getting-started/modeling)
* [RBAC modeling guides](/permify-docs/modeling-guides/rbac/custom-roles)
* [ABAC modeling guides](/permify-docs/modeling-guides/abac/public-private)
* [ReBAC modeling guides](/permify-docs/modeling-guides/rebac/hierarchies)
...
For user data, you can use the [User Create](/docs/extend/events-and-webhooks/events/user/user-create) and [User Update](/docs/extend/events-and-webhooks/events/user/user-update) webhooks to propagate user data. Make these webhooks transactional, but fast, so use something like a durable queue to receive the events. Or if you have Kafka in your system already, [publish them to Kafka](/docs/extend/events-and-webhooks/kafka). Then, use one of the [Permify SDKs](/permify-docs/getting-started/enforcement#sdks) and whenever changes occur, update Permify.

For application-specific entities and relations, use one of the [Permify SDKs](/permify-docs/getting-started/enforcement#sdks) and whenever changes occur, update Permify.
...
* Read about [FusionAuth FGA by Permify](/permify-docs/).
