...
* Update the FusionAuth configuration to ensure that `JS2` is used for all applications and tenants for which `JS1` was used. At the same time, add `JS1` to the **Access token verification keys** list (and/or **Id token verification keys** list) at the application or tenant level (depending on whether application-level JWT override is enabled). This option was added in FusionAuth `1.69.0`.
...
* Update the FusionAuth configuration to ensure that `JS2` is used for all applications and tenants for which `JS1` was used. At the same time, add `JS1` to the **Access token verification keys** list (and/or **Id token verification keys** list) at the application or tenant level.
...
As long as the above recommendations are followed, this problem doesn't arise in the same manner for JWT signing keys because they have a built in grace period: the expiration of the JWTs. You can definitely cause issues by removing a JWT signing key from a tenant, application, or identity provider before all the keys it has signed have expired, but because JWT signing keys are only used by FusionAuth to sign JWTs and have a built-in expiration time, it is easy to use the grace period option above.
...
```shell title="Script to query for keys expiring in 30 days"
#!/bin/bash
...
* JWT configuration
  ...
  * `tenant.jwtConfiguration.accessTokenVerificationKeyIds`
  * `tenant.jwtConfiguration.idTokenKeyId`
  * `tenant.jwtConfiguration.idTokenVerificationKeyIds`
  * `application.jwtConfiguration.accessTokenKeyId`
  * `application.jwtConfiguration.accessTokenVerificationKeyIds`
  * `application.jwtConfiguration.idTokenKeyId`
  * `application.jwtConfiguration.idTokenVerificationKeyIds`
* Entity Type JWT configuration
  * `entityType.jwtConfiguration.accessTokenKeyId`
* External JWT IdP
  * `identityProvider.defaultKeyId`
  * `identityProvider.verificationKeyIds`
* Application SAML v2
  ...
  * `application.samlv2Configuration.logout.verificationKeyIds`
  * `application.samlv2Configuration.verificationKeyIds`
* SAML v2 IdP
