...
Fusionauth exposes metrics in a 'Prometheus Endpoint', but we must generate an API key and give permission to the "get" request to access it.
To do this
...
- Click on the 'GET STARTED FREE' link and complete the registration form to get a 14-day trial. 
  <Aside type="note">
    Make a note hosting location, This will determine the URL you use later, for this guide we use United States (US5-Central),: i.e. us5.datadoghq.com.
  </Aside>
...
- Click on the Ubuntu agent, and it will show the below code.
...
- We will add it to the Fusionauth dockerfile to install and start the agent for us.
...
- Above the paragraph labelled ###### Start FusionAuth App, we add a version of the code we generate from the Datadog site and add it to the docker image.
```DockerFile
 ### NEW FOR DATADOG ###
...
```
- We download the Datadog agent from https://s3.amazonaws.com/dd-agent/scripts/install_script_agent7.sh 
- Set some extra environment variables to  do the installation 
- After installing the Datadog agent, we copy the config(openmetrics.d-conf.yaml),  to handle the open metrics integration.
...
- When the agent is successfully installed and started, you will get a screen that will refresh and indicate that reports from the agent are received. 
...
- Search for open metrics, and click on the tile presented
...
We use the sample config file to configure the open metrics integrations. It was already copied to the docker container when we created it.
``` yaml
init_config:
instances:
  - prometheus_url: http://localhost:9011/api/prometheus/metrics  
...
We added this part to the DockerFile before the section `### start the agent ###`
...
- Make sure of the region. We use "us5.datadoghq.com", but yours might vary depending on the selection when you created your Datadog [account](### Signup for a free Datadog trial account.).
- Click Download Agent Installer.
- If asked, Choose a folder to save the download, else it will be saved in your default downloads folder.
...
- Choose the last 15 minutes for the period. You will see that data being pulled from the Prometheus fusionauth endpoint.
- Following the steps above when we created a [dashboard](### Creating your first Dashboard), you can easily add these metrics to the dashboards required.
...
Run the folliowing command before you try the script:
```bash
...
with ApiClient(configuration) as api_client:
    ...
    print(response)
```    
if you navigate [to](https://us5.datadoghq.com/event/overview) and click on Explorer you will see the event that we logged with the script.
...
We can also use the open telemetry java colletor to push data from the Fusionauth host to Datadog.
...
- This code edits `start.sh`, which is the command run when the container starts, to start FusionAuth using OpenTelemetry. 
...
```
### Visualizin the Opentelemetery Metrics in Datadog
...
- If you followed the guide correctly you will see som metrics on the graps
...
- We looked at using the API in a Python app to push sample events/logs to Datadog.
