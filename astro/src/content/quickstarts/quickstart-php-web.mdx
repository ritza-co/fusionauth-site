...
codeRoot: https://raw.githubusercontent.com/ritza-co/fusionauth-quickstart-php-web/main
---
...
This setup code does a few things:
- Prevent errors from being displayed in the browser, for security.
- Load all vendor modules into PHP so you can use them.
- Read your variable from the `.env` file.
- Start a new session, using HTTP (since you're on localhost), and ensuring your user id cookie will not be accessible by JavaScript.

Next comes the FusionAuth handlers. Add the following lines to the file.
...
- Sends them to the FusionAuth URL if they are not logged in.
- Checks their FusionAuth token if they are not logged in, but are returning from FusionAuth.
- Gets their user details from the FusionAuth token and saves them to the server-side session. This also generates a new session/cookie Id for the user for security. The user then redirects to their account page.

The user's session is linked to their browser automatically by PHP returning a cookie for the site, that is sent with every subsequent request.

The login page handles both the initial user request to start the login process, and the server callback request from FusionAuth to complete the authentication process.
...
This eradicates the session on the server for this user and redirects them back to the homepage.
...
Copy CSS and images from the example app.

```shell
cp complete-application/your-application/public/static/changebank.css your-application/public/static/changebank.css
...
You'll create three pages. The first is the home page, `index.php`. Paste the following code into the file.
...
Next, create an `account.php` file. Paste the following code into the file.
...
It also is visible only to logged-in users. If a session Id is not found, the user is redirected back to login.

Finally, create a `change.php` file. Paste the following code into the file.

<RemoteCode
  url={frontmatter.codeRoot + "/complete-application/your-application/public/change.php"}
  lang="php" />

In addition to verifying login, the code at the top of this page also checks if the CSRF token is valid on POST requests. The CSRF token is hidden in the form in the HTML, `<input type="hidden" name="csrftoken" value="<?= $_SESSION["csrftoken"] ?>" />`. Although any attacker can make a POST request on behalf of a logged-in user, a server will reject a GET request if not made from the same origin (URL). Thus including this CSRF token in your page requires any attacker to make a GET request to obtain it, before making the POST request. This is impossible for them.
...
The HTML at the bottom of the file displays a blank form when first loading the page (GET), or the result of the calculation when returning (POST).

## Run The Application

The application is already being served by Apache in Docker.

Browse to the app at http://localhost:9012. Log in using `richard@example.com` and `password`. The change page allows you to enter a number. Log out and verify that you can't browse to the [account page](http://localhost:9012/account).
