...
Before starting coding, you need to install the PHP module for FusionAuth communication. Run the following code.
...
The login file is based almost exactly on Jerry Hopper's code, but is split into neater function and does two extra things: start a session, and save user details to the session after login.
...
- Starts a new session, using HTTP not HTTPS (since you're on localhost), and ensuring your user Id cookie will not be accessible by JavaScript.
