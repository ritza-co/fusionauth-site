...
<Aside type="note">
  Note that the `phpimage.Dockerfile` in the root directory of the project configures Apache to use the `public` directory as what is available on the web, to hide
  `vendor` and configuration files. If you're not using Docker, you will have to do this yourself.
...
<Aside type="note">
  ...
  Then view the application at the following URL http://localhost:9012.
</Aside>
...
Next comes the FusionAuth handlers. The code, starting from the `getFusionAuthProvider()` function:
...
- Gets their user details from the FusionAuth token and saves them to the server-side session. This also generates a new session/cookie Id for the user for security. The user is then redirected to their account page.
...
Create a `static` directory within the `your-application/public` directory.
...
In you `your-application/public` directory you'll create three more pages. The first is the home page, `index.php`. Paste the following code into the file.
