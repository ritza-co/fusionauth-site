...
Then, update the `Program.cs` file to use the `Startup.cs` file you just created. Replace the `Program.cs` file with the following code:
...
The `Startup.cs` file configures the OIDC middleware to use the FusionAuth OIDC settings from the `appsettings.json` file. The `Program.cs` file configures the application to use the `Startup.cs` file.
