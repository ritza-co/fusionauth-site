...
You create the default Express starter project in a folder called `mysite`. You also add a few npm packages to help with authentication.

```shell
npx express-generator@latest mysite --view=ejs
cd mysite
npm install
...
npm install passport passport-oauth2  # to authenticate with FusionAuth
npm install jwt-decode # to get the email address from the JWT returned by FusionAuth
npm install express-session # to maintain the user's session once logged in
```
...
At the top of `mysite/app.js`, add the following line.
...
- [Passport.js Oauth2](https://www.passportjs.org/packages/passport-oauth2/)
