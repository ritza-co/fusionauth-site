---
title: Express.js
description: Quickstart integration of JavaScript Express.js web application with FusionAuth
...
In this quickstart, you will build an application with Express.js and integrate it with FusionAuth. The application is for [ChangeBank](https://www.youtube.com/watch?v=pkH-kD73QUM), a global leader in converting dollars into coins. It will have areas reserved for logged in users and public-facing sections.

Find the Docker Compose file and source code for the complete application at https://github.com/FusionAuth/fusionauth-quickstart-javascript-express-web.
...
Start with getting FusionAuth up and running and creating a new Express application.
...
Next, you'll set up a basic Express app. While this guide builds a new Express project, you can use the same method to integrate your existing project with FusionAuth.

Create the default Express starter project.

```shell
npx create-express@latest mysite
```
...
```shell
cd mysite
npm install dotenv express-auth express-auth-oauth2 jwt-decode
```

The two Express authentication packages allow you to use FusionAuth without needing to know how OAuth works. The JWT package will provide your app with the user details returned from FusionAuth. The `dotenv` package allows the app to read configuration details from a `.env` environment file.
...
<RemoteCode url="https://raw.githubusercontent.com/FusionAuth/fusionauth-quickstart-javascript-express-web/main/complete-application/.env"
 lang="shell" />

### Preliminary Setup

Before you configure authentication, you need to change the default files created by Express.

Replace the contents of the file `mysite/app/root.tsx` with the following code.

<RemoteCode url="https://raw.githubusercontent.com/FusionAuth/fusionauth-quickstart-javascript-express-web/main/complete-application/app/root.tsx"
 lang="typescript" />
...
Authentication in Express requires two parts:
...
The `app` folder in the `mysite` project holds all your code. In Express, the designated `routes` folder contains your application pages. Let's make a `services` folder to keep your authentication code separate.

```shell
mkdir app/services
```

In this `services` folder, add a file called `session.server.ts` and copy in the following code (this guide follows the example from the [Express Auth README](https://github.com/sergiodxa/express-auth)).

<RemoteCode url="https://raw.githubusercontent.com/FusionAuth/fusionauth-quickstart-javascript-express-web/main/complete-application/app/services/session.server.ts"
 lang="typescript" />

Next, create an authentication file in the services folder, `auth.server.ts`, that uses the session provider you just created.

<RemoteCode url="https://raw.githubusercontent.com/FusionAuth/fusionauth-quickstart-javascript-express-web/main/complete-application/app/services/auth.server.ts"
 lang="typescript" />

In the line `new Authenticator<User>`, you can specify which user details you want to keep in the session. This example uses a single string containing the user's email, but you could define an object with more details or just a user Id. To do so, you would change the definition of `User`, update the object returned in the authentication method (`return token.email`), and update the routes you are about to create to use the new object instead of just an email.
...
<RemoteCode url="https://raw.githubusercontent.com/FusionAuth/fusionauth-quickstart-javascript-express-web/main/complete-application/app/routes/_index.tsx"
 lang="typescript" />
...
Express extends React with a few more custom elements that automatically hide the difference between client and server from the programmer. For example, the line `<Link to="/login" className="button-lg">Login</Link>` allows Express to load the linked page in the background without a separate page refresh.
...
<Aside type="note">
Note that this code runs in a Express `loader` function on the server, not the client. Remember that Express provides two functions that run only on the server: `loader` for GET requests and `action` for POST, PATCH, and UPDATE requests. These functions will always run before any HTML is returned to the client. By contrast, the `default` function (that returns HTML) is exported from a route and runs only in the browser (unless the user browses directly to the route without already being on the site). Since Express mixes server-side and client-side code in the same file, it is essential not to put any authentication handling or secrets in the client-side code.
</Aside>
...
<RemoteCode url="https://raw.githubusercontent.com/FusionAuth/fusionauth-quickstart-javascript-express-web/main/complete-application/app/routes/login.tsx"
 lang="typescript" />
...
<RemoteCode url="https://raw.githubusercontent.com/FusionAuth/fusionauth-quickstart-javascript-express-web/main/complete-application/app/routes/logout.tsx"
 lang="typescript" />
...
<RemoteCode url="https://raw.githubusercontent.com/FusionAuth/fusionauth-quickstart-javascript-express-web/main/complete-application/app/routes/auth.callback.tsx"
 lang="typescript" />
...
<Aside type="tip">
If you are wondering why `auth.callback.tsx` has two dots, it's because Express routing treats the first dot as a slash in the URL, `/auth/callback`.
</Aside>
...
<RemoteCode url="https://raw.githubusercontent.com/FusionAuth/fusionauth-quickstart-javascript-express-web/main/complete-application/app/routes/account.tsx"
 lang="typescript" />
...
<RemoteCode url="https://raw.githubusercontent.com/FusionAuth/fusionauth-quickstart-javascript-express-web/main/complete-application/app/routes/change.tsx"
 lang="typescript" />

This page has identical Express and authentication functionality to the Account page and some pure React code that updates the DOM whenever the state changes.
...
- [Express Docs](https://express.run/docs)
- [Passport.js authentication concepts](https://www.passportjs.org/concepts/authentication/downloads/html/)
- [Express Auth](https://github.com/sergiodxa/express-auth)
- [Express-Auth-Oauth2 Docs](https://github.com/sergiodxa/express-auth-oauth2)
