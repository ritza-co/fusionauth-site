...
cd "$SCRIPT_DIR"
docker run --network host --name playwright-test --rm -e NODE_PATH=/usr/lib/node_modules -v "$SCRIPT_DIR":/tests mcr.microsoft.com/playwright:v1.61.1 bash -c "npm install -g @playwright/test && cd /tests && playwright test integration.spec.js"
TEST_EXIT_CODE=$?
