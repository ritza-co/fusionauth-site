...
- After completeing the setup you will be redirected to the 'agent intstallation' page.  we will use the agent in the next section
...
We will update FusionAuth DockerFile by adding the ububt agent in there
So click on the ubuntu agent and add this to the DockerFile..... describer
```bash
DD_API_KEY=5b4877f6ec8ec6cdbca8180b42579d8a DD_SITE="us5.datadoghq.com" bash -c "$(curl -L https://s3.amazonaws.com/dd-agent/scripts/install_script_agent7.sh)"
```
