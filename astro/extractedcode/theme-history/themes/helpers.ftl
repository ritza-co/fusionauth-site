        ...
        [#list actionURLs as url]
            ...
            [#if actionURL?has_content]
                ...
                [#if !actionURL?contains("tenantId")]
                    ...
                    [/#if]
                [/#if]
                [#if redirectURI?? && !actionURL?contains("redirect_uri")]
                    [#if actionURL?contains("?")]
                        [#local actionURL = actionURL + "&redirect_uri=${redirectURI?url('UTF-8')}"/]
                    [#else]
                        [#local actionURL = actionURL + "?redirect_uri=${redirectURI?url('UTF-8')}"/]
                    [/#if]
                [/#if]
...
[#-- Render checkbox used to determine whether the form submit should update password--]
  ...
  <div id="password-fields" class="slide-open ${(editPasswordOption == "update")?then('', 'hidden')} flex flex-col space-y-3">
      ...
      [#if field.confirm]
          [@customField field=field key="confirm.${field.key}" autofocus=false placeholder="[confirm]${field.key}" label=theme.optionalMessage("confirm.${field.key}")/]
      [/#if]
  </div>
[/#macro]
